...
import { SPEC_COLUMNS, SPEC_FILTER_STATUSES } from '@/lib/kanban'
...
export default function StoriesPage() {
  ...
  const [localSpecs, setLocalSpecs] = useState<IndexItem[] | null>(null)
  ...
  const specs = localSpecs ?? allSpecs
  ...
  const handleDragEnd = useCallback(
    async (event: DragEndEvent) => {
      ...
      const targetColumn = SPEC_COLUMNS.find((c) => c.id === String(over.id))
      if (!targetColumn) return

      const newStatus = targetColumn.dropStatus
      const prev = specs

      setLocalSpecs(prev.map((item) => (item.id === itemId ? { ...item, status: newStatus } : item)))
              ...
              <KanbanColumn
                ...
                renderCard={(ki) => (
                  <SortableStoryCard
                    ...
                    item={ki.item}
                    repo={ki.repo}
                    activeChanges={inChangeMap.get(ki.item.id)}
                    onClick={() => openItem(ki.item)}
